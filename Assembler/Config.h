...
typedef struct Symbol{
    ...
    uint16_t       address; /* Symbol address      */
    struct Symbol* next;    /* Next symbol pointer */
} Symbol;

typedef struct SymbolTable {
    Symbol*  head;       /* First symbol entry */
    Symbol*  tail;       /* Last symbol entry  */
    uint16_t size;       /* Number of symbols  */
    uint16_t romAddress; /* Next ROM address   */
    uint16_t ramAddress; /* Next RAM address   */
} SymbolTable;
